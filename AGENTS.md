...
This repository is a dependency-free, offline web application. `index.html` contains only the page shell, the `ZW` namespace forwarding list, and startup assembly (boot/init); `app.css` owns the visual design. Browser modules live in `js/` and are loaded in the explicit order declared near the top of `index.html`: plugins first (`_ns.js`, seeded random, utilities, core store, icons, sound, spin, multi-face), then business domains (`wheels`, `state`, `growth`, `navigation`, `chart`, `flow`, `storage`, `wheelview`, `views`, `actions`). Preserve that order when adding or splitting modules: earlier modules export onto `ZW` for later ones to destructure, while references to later-owned symbols must go through runtime `ZW.x` property access. The mutable `state`/`ui`/`NAV` singletons are shared by reference and only ever replaced in place (`loadState`). `sw.js` caches a hardcoded file list that must stay in sync with the `index.html` script tags — `node _dev/run.js` checks that drift.
